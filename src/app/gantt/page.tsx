...
function GanttChart({ tasks }: { tasks: Task[] }) {
  ...
  const numberOfDays = activeMonths.map((month) =>
    new Date(2025, month + 1, 0).getDate()
  );

  const startDate = new Date(2025, activeMonths[0] + 1, 1);
  const numberOfWeeksTotal = Math.ceil(
    numberOfDays.reduce((acc, days) => acc + days, 0) / 7
  );
    ...
    <ScrollArea className="max-w-[90%]">
      <div className="grid grid-cols-[auto_1fr] grid-rows-[auto_1fr] gap-y-2">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex">
            {activeMonths.map((month) => (
              <div
                key={month}
                className="text-sm bg-gray-200 mr-2 text-center rounded"
                style={{
                  width: `${numberOfDays[activeMonths.indexOf(month)]}rem`,
                }}
              >
          ...
          <div className="flex justify-between">
            {Array.from({ length: numberOfWeeksTotal }).map((_, index) => {
              ...
              return (
                <div
                  key={index}
                  className="text-sm w-[7rem] bg-gray-200 mr-2 rounded text-center"
                >
                  {`${weekStart.getDate()}/${
        ...
        <div className="flex flex-col gap-2">
          {tasks.map((task) => {
            const start =
              (task.start.getTime() - new Date(2025, 3, 1).getTime()) /
              (1000 * 60 * 60 * 24);
            const end =
              (task.end.getTime() - new Date(2025, 3, 1).getTime()) /
              (1000 * 60 * 60 * 24);
            return (
              <div
                key={task.id}
              ...
              >
                {Array.isArray(task.assignee) &&
                  task.assignee.map((assignee, i) => (
                    <Avatar
                      className="h-6 w-6 border-[3px] border-white"
                      key={i}
                      style={{
                        position: "absolute",
                        top: "50%",
                        ...
                        filter: `brightness(${
                          i === task.assignee.length - 1 ? 1 : 0.8
                        })`,
                      }}
                    >
